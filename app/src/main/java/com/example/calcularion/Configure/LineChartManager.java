...
public class LineChartManager {
     ...
     * @return LineData
     */
    public static LineData initSingleLineChart(Context context, LineChart mLineChart, ArrayList<String> xva,int count, float[] datas,int flag) {

        ArrayList<String> xValues = new ArrayList<String>();
        for (int i = 0; i < count; i++) {
            // x轴显示的数据，这里默认使用数字下标显示
           // xValues.add((i) + ":00");
            xValues.add(xva.get(i));
        }
        ...
        //设置折线的样式
        if (flag==1){
            LineDataSet dataSet = new LineDataSet(yValues, "收入");
            //用y轴的集合来设置参数
            dataSet.setLineWidth(1.75f); // 线宽
            dataSet.setCircleSize(2f);// 显示的圆形大小
            dataSet.setColor(Color.rgb(30,144,255));// 折线显示颜色
            dataSet.setCircleColor(Color.rgb(30,144,255));// 圆形折点的颜色
            dataSet.setHighLightColor(Color.GREEN); // 高亮的线的颜色
            dataSet.setHighlightEnabled(true);
            dataSet.setValueTextColor(Color.rgb(30,144,255)); //数值显示的颜色
            ...
            LineDataSet dataSet = new LineDataSet(yValues, "支出");
            //用y轴的集合来设置参数
            dataSet.setLineWidth(1.75f); // 线宽
            dataSet.setCircleSize(2f);// 显示的圆形大小
            dataSet.setColor(Color.rgb(255, 0, 0));// 折线显示颜色
            dataSet.setCircleColor(Color.rgb(255, 0, 0));// 圆形折点的颜色
            dataSet.setHighLightColor(Color.GREEN); // 高亮的线的颜色
            dataSet.setHighlightEnabled(true);
            dataSet.setValueTextColor(Color.rgb(255, 0, 0)); //数值显示的颜色
            dataSet.setValueTextSize(8f);     //数值显示的大小
     ...
     * @Description:创建两条折线
     */
    public static LineData initDoubleLineChart(Context context, LineChart mLineChart,ArrayList<String> xva, int count, float[] datas1, float[] datas2) {

        ArrayList<String> xValues = new ArrayList<String>();
        for (int i = 0; i < xva.size(); i++) {
            // x轴显示的数据，这里默认使用数字下标显示
//            xValues.add((i) + ":00");
            xValues.add(xva.get(i));
        }
        ...
        dataSet1.setColor(Color.rgb(30,144,255));
        dataSet1.setCircleColor(Color.rgb(30,144,255));
        dataSet1.setHighLightColor(Color.GREEN);
        dataSet1.setHighlightEnabled(true);
        dataSet1.setValueTextColor(Color.rgb(30,144,255));
